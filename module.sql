...
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_ADMIN_CREATE_USER','The administrator may want to <a href="%s">Create Users</a>, or this can be left up to instructors using the Enrollment Manager, or students can register themselves if enabled in the System Preferences. Open the Users tab above, then open the Create User tab in the sub-menu.',NOW(),'');
...
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_READ_HANDBOOK','Be sure to find the %s in the links at the bottom of the screen for a full description of other features. Enter a keyword search to find information about any feature. Also notice the context sensitive ATutor Handbook tab that often appears alongside or above various tools for specific information about using that tool.',NOW(),'');
...
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_CREATE_CONTENT','You can use ATutor to <a href="%s">Create Content</a> for your course. Content can include movies, images, slides, documents or text, among other content types. Under the Manage tab above, open Create in the Content sub-menu to add new pages. Also notice the Content Navigation block to the side, and the small toobar at the top that can be used to quickly add folders, pages, and to edit the menu below. Also the Editor Toolbar appears with content that can be edited, for quick access to a range of content management tools. ',NOW(),'');
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_COURSE_TOOLS','You can select from a range of <a href="%s">Course Tools</a> to add particular features to a course. Tools can be added to the Main Tabs above, to the menu blocks at the side, or added as icons or boxes on the course home page. To manage the tools used in your course open Manage above, then open the Course Tools sub-menu.',NOW(),'');
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_ADD_USERS','After a course is setup and content added, the next step is often to <a href="%s">Create a Course List</a>, and enroll students. Students can be enrolled manually typing them in one at a time, by importing a list in a CSV text file, by adding students from those registered on the system, or allowing students to enroll themselves. To add students to your course open the Manage tab above, then open the Enrolment sub-menu. ',NOW(),'');
...
INSERT INTO `language_text` VALUES ('en', '_msgs','AT_HELP_CREATE_BACKUP','To protect your course <a href="%s">Create a Backup</a> that can be stored on your own computer, and restored in whole or in part whenever needed. Use backups to create a new session of a course, or to move a course to a different ATutor site. Also notice the tools for packaging (importing/exporting) content and tests, for exporting test data, and for archiving forums, as other ways to backup your course.',NOW(),'');